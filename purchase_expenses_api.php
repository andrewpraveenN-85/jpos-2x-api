<?php
// purchase_expenses_api.php - Purchase Expenses API
header('Content-Type: application/json');

error_reporting(E_ALL);
ini_set('display_errors', 0);

function getAllHeadersSimple() {
    if (function_exists('getallheaders')) return getallheaders();
    $headers = [];
    foreach ($_SERVER as $name => $value) {
        if (substr($name, 0, 5) == 'HTTP_') {
            $headers[str_replace(' ', '-', ucwords(strtolower(str_replace('_', ' ', substr($name, 5)))))] = $value;
        } elseif ($name == 'CONTENT_TYPE') {
            $headers['Content-Type'] = $value;
        } elseif ($name == 'CONTENT_LENGTH') {
            $headers['Content-Length'] = $value;
        }
    }
    return $headers;
}

try {
    if ($_SERVER['REQUEST_METHOD'] !== 'GET') {
        throw new Exception('Only GET method is allowed', 405);
    }

    $headers = getAllHeadersSimple();

    $db_host = $headers['X-DB-Host'] ?? $_SERVER['HTTP_X_DB_HOST'] ?? ($headers['x-db-host'] ?? 'localhost');
    $db_user = $headers['X-DB-User'] ?? $_SERVER['HTTP_X_DB_USER'] ?? ($headers['x-db-user'] ?? '');
    $db_pass = $headers['X-DB-Pass'] ?? $_SERVER['HTTP_X_DB_PASS'] ?? ($headers['x-db-pass'] ?? '');
    $db_name = $headers['X-DB-Name'] ?? $_SERVER['HTTP_X_DB_NAME'] ?? ($headers['x-db-name'] ?? '');
    $db_port = $headers['X-DB-Port'] ?? $_SERVER['HTTP_X_DB_PORT'] ?? ($headers['x-db-port'] ?? 3306);

    if (empty($db_user) || empty($db_name)) {
        throw new Exception('Database configuration incomplete. Required: X-DB-User and X-DB-Name', 400);
    }

    $mysqli = @new mysqli($db_host, $db_user, $db_pass, $db_name, (int)$db_port);
    if ($mysqli->connect_error) {
        throw new Exception('Database connection failed: ' . $mysqli->connect_error, 500);
    }
    $mysqli->set_charset('utf8mb4');

    // Filters & pagination
    $id = isset($_GET['id']) && $_GET['id'] !== '' ? (int)$_GET['id'] : null;
    $title = isset($_GET['title']) && $_GET['title'] !== '' ? trim($_GET['title']) : null;
    $user_id = isset($_GET['user_id']) && $_GET['user_id'] !== '' ? (int)$_GET['user_id'] : null;
    $supplier_id = isset($_GET['supplier_id']) && $_GET['supplier_id'] !== '' ? (int)$_GET['supplier_id'] : null;
    $payment_type = isset($_GET['payment_type']) && $_GET['payment_type'] !== '' ? (int)$_GET['payment_type'] : null;
    $date_from = isset($_GET['date_from']) && $_GET['date_from'] !== '' ? $_GET['date_from'] : null;
    $date_to = isset($_GET['date_to']) && $_GET['date_to'] !== '' ? $_GET['date_to'] : null;
    $page = isset($_GET['page']) ? max(1, (int)$_GET['page']) : 1;
    $per_page = isset($_GET['per_page']) ? (int)$_GET['per_page'] : 50;
    if ($per_page < 1 || $per_page > 500) $per_page = 50;

    $where = [];
    $params = [];
    $types = '';

    if ($id !== null) { $where[] = 'pe.id = ?'; $params[] = $id; $types .= 'i'; }
    if ($title !== null) { $where[] = 'pe.title LIKE ?'; $params[] = '%' . $title . '%'; $types .= 's'; }
    if ($user_id !== null) { $where[] = 'pe.user_id = ?'; $params[] = $user_id; $types .= 'i'; }
    if ($supplier_id !== null) { $where[] = 'pe.supplier_id = ?'; $params[] = $supplier_id; $types .= 'i'; }
    if ($payment_type !== null) { $where[] = 'pe.payment_type = ?'; $params[] = $payment_type; $types .= 'i'; }
    if ($date_from !== null) { $where[] = 'pe.expense_date >= ?'; $params[] = $date_from; $types .= 's'; }
    if ($date_to !== null) { $where[] = 'pe.expense_date <= ?'; $params[] = $date_to; $types .= 's'; }

    $where_clause = '';
    if (!empty($where)) $where_clause = 'WHERE ' . implode(' AND ', $where);

    // Count total
    $count_sql = "SELECT COUNT(*) as total FROM purchase_expenses pe $where_clause";
    $count_stmt = $mysqli->prepare($count_sql);
    if (!$count_stmt) throw new Exception('Failed to prepare count query: ' . $mysqli->error, 500);
    if (!empty($params)) $count_stmt->bind_param($types, ...$params);
    $count_stmt->execute();
    $count_res = $count_stmt->get_result()->fetch_assoc();
    $total = $count_res ? (int)$count_res['total'] : 0;
    $count_stmt->close();

    $offset = ($page - 1) * $per_page;

    $fields = [
        'pe.id','pe.title','pe.amount','pe.remark','pe.expense_date','pe.payment_type','pe.reference','pe.user_id','pe.supplier_id','pe.created_at','pe.updated_at',
        's.name as supplier_name','s.email as supplier_email','s.phone_number as supplier_phone','s.address as supplier_address','s.status as supplier_status'
    ];

    $select_sql = 'SELECT ' . implode(', ', $fields) . " FROM purchase_expenses pe LEFT JOIN suppliers s ON pe.supplier_id = s.id $where_clause ORDER BY pe.id DESC LIMIT ? OFFSET ?";
    $stmt = $mysqli->prepare($select_sql);
    if (!$stmt) throw new Exception('Failed to prepare select query: ' . $mysqli->error, 500);

    $all_params = $params;
    $all_types = $types . 'ii';
    $all_params[] = $per_page;
    $all_params[] = $offset;
    if (!empty($all_params)) $stmt->bind_param($all_types, ...$all_params);

    $stmt->execute();
    $res = $stmt->get_result();

    $items = [];
    while ($row = $res->fetch_assoc()) {
        $supplier = null;
        if ($row['supplier_id'] !== null) {
            $supplier = [
                'id' => (int)$row['supplier_id'],
                'name' => $row['supplier_name'],
                'email' => $row['supplier_email'],
                'phone_number' => $row['supplier_phone'],
                'address' => $row['supplier_address'],
                'status' => $row['supplier_status'] !== null ? (int)$row['supplier_status'] : null
            ];
        }

        $items[] = [
            'id' => (int)$row['id'],
            'title' => $row['title'],
            'amount' => isset($row['amount']) ? (float)$row['amount'] : null,
            'remark' => $row['remark'],
            'expense_date' => $row['expense_date'],
            'payment_type' => isset($row['payment_type']) ? (int)$row['payment_type'] : null,
            'reference' => $row['reference'],
            'user_id' => (int)$row['user_id'],
            'supplier_id' => $row['supplier_id'] !== null ? (int)$row['supplier_id'] : null,
            'supplier' => $supplier,
            'created_at' => $row['created_at'],
            'updated_at' => $row['updated_at']
        ];
    }

    $stmt->close();
    $mysqli->close();

    $response = [
        'success' => true,
        'message' => 'Purchase expenses retrieved',
        'data' => [
            'filters' => ['id' => $id, 'title' => $title, 'user_id' => $user_id, 'supplier_id' => $supplier_id, 'payment_type' => $payment_type, 'date_from' => $date_from, 'date_to' => $date_to],
            'pagination' => [
                'current_page' => $page,
                'per_page' => $per_page,
                'total_items' => $total,
                'total_pages' => $per_page > 0 ? ceil($total / $per_page) : 0
            ],
            'items' => $items
        ]
    ];

    http_response_code(200);
    echo json_encode($response, JSON_PRETTY_PRINT);
    exit;

} catch (Exception $e) {
    $status_code = $e->getCode();
    if ($status_code < 100 || $status_code > 599) $status_code = 500;
    $error = [
        'error' => true,
        'status_code' => $status_code,
        'message' => $e->getMessage()
    ];
    http_response_code($status_code);
    echo json_encode($error, JSON_PRETTY_PRINT);
    exit;
}
